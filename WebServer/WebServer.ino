...
// code for /pwm code
uint8_t motorApin1 = D0;
uint8_t motorApin2 = D1;
uint8_t motorBpin1 = D2;
uint8_t motorBpin2 = D3;

// check if string only has numbers
bool isPWM(String &arg){
  for(char c : arg){
    // Serial.println("isPWM char: "+c);
    if(!isDigit(c) ){
      return false;
    }
  }
...
// api for /pwm which reads in argument localhost/pwm?PWMAFOR=255&PWMBFOR=255
void handlePWM(){
  for(int i=0; i< server.args(); i++){
    // Serial.println(server.argName(i) + ": " + server.arg(i) );
    String netName = server.argName(i);
    String netValue = server.arg(i);
    int netPWM = 0;
    
    if(!isPWM(netValue) ){
      // Serial.println("Invalid PWM: "+netValue);
      continue;
    }
    

    netPWM = netValue.toInt();
    
    if(netName == "PWMAFOR"){
      analogWrite(motorApin2, 0);
      analogWrite(motorApin1, netPWM);
    } else if(netName == "PWMABAC"){
      analogWrite(motorApin1, 0);
      analogWrite(motorApin2, netPWM);
    } else if(netName == "PWMBFOR"){
      analogWrite(motorBpin2, 0);
      analogWrite(motorBpin1, netPWM);
    } else if(netName == "PWMBBAC"){
      analogWrite(motorBpin1, 0);
      analogWrite(motorBpin2, netPWM);
    } else{
      // Serial.println("Invalid argument:{" + netName + ": "+ netValue +"}");
    }
  }
...
void setup(void) {
  //setting up my pins for /pwm API
  ...
  server.on("/pwm", HTTP_POST, handlePWM ); // handlePWM is the function being used to control motor pwm
