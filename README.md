# ESP32 webapp with PWM controll for GPIO pins
The main purpose of this project is to host a webapp on the ESP32 and to be able to controll the GPIO calls with API calls to the ESP32. I want to create a RC car that I will be able to control via WIFI. Most of the code is working, I just want to improve on reliability.

For this project, I am using the Arduino-ESP32 webserver example code. This code allows me to upload html files, so I will be able to create my own index.html. And with my own index.html file, I will be able to use users browser to load the gamepad-api, and users will be able to control the ESP32 with a game controller.

# Details
## Hardware
- This code is being tested on the XIAO esp32c6 microcontroller. Probably cheaper on amazon - [Shop $6](https://wiki.seeedstudio.com/xiao_esp32c6_getting_started/)
- I am using pins D0 and D1 to controll motor A of the RC car.
- I am using pins D2 and D3 to controll motor B of the RC car.
- XIAO ESP32C6 pinout:
<img src="xiao_esp32c6_pinout.webp">

## Current API that is implemented
  -  http://[`device-ip-address`]/pwm?PWMAFOR=255&PWMBFOR=255
  - http://[`device-ip-address`]/pwm?PWMABAC=255&PWMBBAC=255
  - device-ip-address is the ip address of the esp32 when it connects to your home network
  - The values sent to the api can be set from 0 to 255. 0 will be off, and 255 will be the max value for GPIO pins on the XIAO esp32.

## Uploading index.html to esp32 web fileserver
- all files can be found in folder named `data`
- upload index.html
